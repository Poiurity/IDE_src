...
float dist_min, dist_max, dist_raw, dist_prev, dist_ema, prev_ema; // unit: mm
...
// get a distance reading from the USS
  ...
  dist_ema = Alpha * pow((1-Alpha),2)* prev_ema;
  prev_ema = dist_ema;
...
// adjust servo position according to the USS read value
  ...
  else if(dist_raw > 355.0){
     myservo.writeMicroseconds(_DUTY_MAX);
  }
  else {
    myservo.writeMicroseconds(1000.0 + 1000.0 * ((dist_raw - 180.0) / 180.0));
  }
